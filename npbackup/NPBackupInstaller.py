...
INSTALL_TO = "{}{}{}".format(os.environ.get("PROGRAMFILES", None), os.sep, PROGRAM_DIRECTORY)
...
def install(config_file=None):
    ...
    exit_code, output = command_runner('schtasks /END /TN "{}"'.format(PROGRAM_NAME), valid_exit_codes=[0, 1], windows_no_window=True, encoding='cp437')
    if exit_code != 0:
        logger.error("Could not terminate currant scheduled task {}:\{}".format(PROGRAM_NAME, output))
    ...
    for file in FILES_TO_COPY:
        ...
        try:
            ...
            logger.error("Could not copy file {} to {}: {}".format(source, destination ,exc))
            return False
    ...
    for dir in DIRS_TO_COPY:
        ...
        try:
            ...
            logger.error("Could not copy directory {} to {}: {}".format(source, destination ,exc))
            return False
    ...
    if not config_file:
        ...
        logger.info("Running configuration from {}".format(NPBACKUP_INSTALLED_EXECUTABLE))
        exit_code, output = command_runner("\"{}\" --config-gui --config-file \"{}\"".format(NPBACKUP_INSTALLED_EXECUTABLE, config_file), shell=False, timeout=3600)
        if exit_code != 0:
    ...
    # Create task
    exit_code, output = command_runner("\"{}\" --create-scheduled-task 15".format(NPBACKUP_INSTALLED_EXECUTABLE, shell=False, timeout=300))
    if exit_code != 0:

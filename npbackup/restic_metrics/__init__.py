...
def restic_output_2_metrics(restic_result, output, labels = None):
    ...
    #for line in output:
        matches = re.match("Files:\s+(\d+)\snew,\s+(\d+)\schanged,\s+(\d+)\sunmodified", line, re.IGNORECASE)
        if matches:
            try:
                metrics.append("restic_repo_files{{{},state=\"new\"}} {}".format(labels, matches.group(1)))
                metrics.append("restic_repo_files{{{},state=\"changed\"}} {}".format(labels, matches.group(2)))
                metrics.append("restic_repo_files{{{},state=\"unmodified\"}} {}".format(labels, matches.group(3)))
            except IndexError:
                logger.warning("Cannot parse restic log for files")
                errors = True

        matches = re.match("Dirs:\s+(\d+)\snew,\s+(\d+)\schanged,\s+(\d+)\sunmodified", line, re.IGNORECASE)
        if matches:
            try:
                metrics.append("restic_repo_dirs{{{},state=\"new\"}} {}".format(labels, matches.group(1)))
                metrics.append("restic_repo_dirs{{{},state=\"changed\"}} {}".format(labels, matches.group(2)))
                metrics.append("restic_repo_dirs{{{},state=\"unmodified\"}} {}".format(labels, matches.group(3)))
            except IndexError:
                logger.warning("Cannot parse restic log for dirs")
                errors = True
        
        matches = re.match("Added to the repo.*:\s([-+]?(?:\d*\.\d+|\d+))\s(\w+)\s+\((.*)\sstored\)", line, re.IGNORECASE)
        if matches:
            try:
                ...
                try:
                    ...
                    metrics.append("restic_repo_size_bytes{{{},state=\"new\"}} {}".format(labels, value))
                except TypeError:
                    logger.warning("Cannot parse restic values from added to repo size log line")
                    errors = True
                stored_size = matches.group(3)
                try:
                    stored_size = int(BytesConverter(stored_size))
                    metrics.append("restic_repo_size_files_stored_bytes{{{},state=\"new\"}} {}".format(labels, stored_size))
                except TypeError:
                    logger.warning("Cannot parse restic values from added to repo stored_size log line")
                    errors = True
            except IndexError as exc:
                logger.warning("Cannot parse restic log for added data: {}".format(exc))
                errors = True

        matches = re.match("processed\s(\d+)\sfiles,\s([-+]?(?:\d*\.\d+|\d+))\s(\w+)\sin\s((\d+:\d+:\d+)|(\d+:\d+)|(\d+))", line, re.IGNORECASE)
        if matches:
            try:
                metrics.append("restic_repo_files{{{},state=\"total\"}} {}".format(labels, matches.group(1)))
                size = matches.group(2)
                unit = matches.group(3)
                try:
                    value = int(BytesConverter("{} {}".format(size, unit)))
                    metrics.append("restic_repo_size_bytes{{{},state=\"total\"}} {}".format(labels, value))
                except TypeError:
                ...
                try:
                    metrics.append("restic_backup_duration_seconds{{{},action=\"backup\"}} {}".format(labels, int(seconds_elapsed)))
                except ValueError:
                ...
                errors = True
        matches = re.match("Failure|Fatal|Unauthorized|no such host|s there a repository at the following location\?", line, re.IGNORECASE)
        if matches:
            try:
                logger.debug("Matcher found error: \"{}\ in line \"{}\".".format(matches.group(), line))
            except IndexError as exc:
                logger.error("Trace:", exc_info=True)
            errors = True


    metrics.append("restic_backup_failure{{{},timestamp=\"{}\"}} {}".format(labels, int(timestamp_get()), 1 if errors else 0))
...
def upload_metrics(destination, authentication, metrics):
    try:
        ...
        result = requests.post(destination, headers=headers, data=data, auth=authentication)
        if result.status_code == 200:
            logger.info("Metrics pushed succesfully.")
        else:
            logger.warning("Could not push metrics: {}: {}".format(result.reason, result.text))
...
if __name__ == "__main__":
    parser = ArgumentParser(prog="restic_log_exporter.py", description="Restic instance prometheus exporter")
    ...
    if not os.path.isfile(log_file):
        logger.error("Restic log file (restic command output) {} does not exist.".format(log_file))
        sys.exit(1)
    ...
    try:
        with open(log_file, "r") as file_handle:
            errors, metrics = restic_output_2_metrics(output=file_handle.readlines(), labels=labels)
        if errors:
        ...
        try:
            ...
            logger.error("Cannot write restic metrics file {}: {}".format(destination_file, exc))
            sys.exit(3)

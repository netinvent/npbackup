...
# Let's keep a global handle to found winow, so we can reuse it later
HANDLE = None


def handle_current_window(action: str = "minimize", use_last: bool = True) -> None:
    """
    ...
    try:
        if os.name == "nt":
            ...
            if use_last and HANDLE:
                hWnd = HANDLE
                logger.debug("Reusing last used handle for current window: %s", HANDLE)
            else:
                ...
                HANDLE = hWnd
                logger.debug("Last used handle for current window: %s", HANDLE)

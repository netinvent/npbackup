    ...
    try:
        ...
        try:
            if config_dict["prometheus"]["http_username"]:
                ...
                config_dict["prometheus"]["http_username"], AES_KEY, ID_STRING, ID_STRING
            )
        ...
        try:
            if config_dict["prometheus"]["http_password"]:
                ...
                config_dict["prometheus"]["http_password"], AES_KEY, ID_STRING, ID_STRING
            )
        except KeyError:
            logger.error("No encrypted http password available.")
            logger.debug("Trace", exc_info=True)
    except ValueError:
        logger.error("Cannot decrypt this configuration file. Has the AES key changed ?")
        sys.exit(11)
    except TypeError:
...
def encrypt_data(config_dict):
    try:
        if config_dict["repo"]["repository"] and not config_dict["repo"]["repository"].startswith(ID_STRING):
        ...
        logger.debug("Trace", exc_info=True)
    try:
        if config_dict["repo"]["password"] and not config_dict["repo"]["password"].startswith(ID_STRING):
    ...
    try:
        if config_dict["prometheus"]["http_username"] and not config_dict["prometheus"]["http_username"].startswith(ID_STRING):
        ...
        logger.debug("Trace", exc_info=True)
    try:
        if config_dict["prometheus"]["http_password"] and not config_dict["prometheus"]["http_password"].startswith(ID_STRING):
...
def is_encrypted(config_dict):
    try:
        if isinstance(config_dict["repo"]["repository"], str) and config_dict["repo"]["repository"].startswith(ID_STRING) and \
        isinstance(config_dict["repo"]["password"], str) and config_dict["repo"]["password"].startswith(ID_STRING) and \
        isinstance(config_dict["prometheus"]["http_username"], str) and config_dict["prometheus"]["http_username"].startswith(ID_STRING) and \
        isinstance(config_dict["prometheus"]["http_password"], str) and config_dict["prometheus"]["http_password"].startswith(ID_STRING):
            return True
        return False
    except KeyError:
        return False
